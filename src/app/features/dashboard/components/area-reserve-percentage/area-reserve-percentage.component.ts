...
import { Contribution, ContributionDetail, DisplayAreaReserve } from '../../../../shared/models/contribution.model';
...
@Component({
  selector: 'app-area-reserve-percentage',
  standalone: true,
  imports: [
    CommonModule,
    HttpClientModule,
  ...
  templateUrl: './area-reserve-percentage.component.html',
  styleUrls: ['./area-reserve-percentage.component.scss'],
})
export class AreaReservePercentageComponent implements OnInit {
  isModalVisible = false;
  fundForm!: FormGroup;

  // This ID is used to filter for "Area Reserve Fund" type contributions.
  // NOTE: Please adjust this ID if it's different in your backend.
  private readonly AREA_RESERVE_TYPE_ID = 4;

  stats: StatCard[] = [
    { title: 'کل درصد صندوق ذخیره مناطق', value: '0', icon: 'percentage' },
    { title: 'میانگین درصد', value: '0%', icon: 'percentage' },
    { title: 'مناطق تحت پوشش', value: '0', icon: 'file-text' },
  ];

  // Data stores
  displayReserves: DisplayAreaReserve[] = [];
  ...
  private processAndSetDisplayData(contribs: Contribution[], details: ContributionDetail[]): void {
    const regionMap = new Map(this.regions.map((r) => [r.id, r.name]));

    const areaReserveContributions = contribs.filter(
      (c) => c.contribution_type_id === this.AREA_RESERVE_TYPE_ID
    );

    this.displayReserves = areaReserveContributions.map((c) => {
      const detail = details.find(d => d.contribution_id === c.id);
      const partyName = regionMap.get(c.party_id) || 'ناشناخته';
      return {
        ...c,
  ...
  private updateStats(): void {
    ...
    const uniqueRegions = new Set(this.displayReserves.map(s => s.party_id)).size;
    this.stats[2].value = uniqueRegions.toString();
  ...
  handleOk(): void {
    if (this.fundForm.valid) {
      ...
      const contributionPayload = {
        party_type: 'region', // This is always region for this component
        party_id: formValue.party_id,
        fin_year: formValue.fin_year,
        contribution_type_id: this.AREA_RESERVE_TYPE_ID,
      ...
      this.http.post<Contribution>(this.contributionApi, contributionPayload).subscribe({
        next: (newContribution) => {
          const detailPayload = {
            ...
            party_id: newContribution.party_id,
            amount: 0,
            percentage: formValue.percentage,
            amnt_or_prcntg: true, // Area reserve is always a percentage
          };
